...
if ! rtk grep -q "hostname: live.priceb.tc" "$config_path"; then
  backup_suffix="$(rtk date +%Y%m%d%H%M%S)"
  rtk sudo cp "$config_path" "${config_path}.backup-live-${backup_suffix}"
  rtk sudo sed -i "/  - service: http_status:404/i\\  - hostname: live.priceb.tc\\n    service: http://127.0.0.1:3466" "$config_path"
fi
...
if [[ "$dns_ready" == "true" ]]; then
  ...
  rtk echo "Verify proxied CNAME records for @, www and live target ${tunnel_target}."
else
  rtk echo "Cloudflare DNS is not authoritative for ${domain} yet."
  rtk echo "Add ${domain} to the Cloudflare account, then create proxied CNAME records for @, www and live target ${tunnel_target}."
fi
