...
function guideMarkup(guide) {
  const api = guide.path === "/bitcoin-price-updates" ? `<section><h2>Public price API</h2><p>GET <a href="/api/price?currency=USD">/api/price?currency=USD</a> returns a timestamped observation. <a href="/api">Read the Bitcoin Price API documentation</a> for field definitions and a response example. Status is live or stale; no available snapshot returns HTTP 503 with PRICE_UNAVAILABLE.</p><p>GET /api/history?currency=USD&amp;range=24h returns timestamped points, range, currency, cachedAt and source. Supported ranges: 1h, 24h, 7d. GET <a href="/api/currencies">/api/currencies</a> lists supported currency codes. Currency defaults to USD; history range defaults to 24h. Invalid currencies or ranges return HTTP 400. Rate limits can return HTTP 429; back off and reuse your recent response.</p><p>Use ordinary GET requests. Avoid crawling live streams, payment, session or admin endpoints. Check timestamps on every observation.</p></section>` : "";
...
function homeFallback() {
  return `<div class="public-site static-fallback">
    ...
    <header class="public-header site-header">${brandMarkup()}<nav aria-label="Primary navigation"><a href="#market">Price</a><a href="#formats">Widgets</a><a href="/#sats-bid">Sponsors</a></nav><a class="action-link" href="/studio">Create a widget ↗</a></header>
...
function apiDocumentation() {
  ...
  return `<div class="public-site"><header class="public-header site-header">${brandMarkup()}<nav><a href="/">Bitcoin price now</a><a href="/bitcoin-price-updates">Price methodology</a></nav></header><main class="public-main guide-content" id="main-content"><h1>PRICEB.TC Bitcoin Price API</h1><p>The free public JSON API returns the Bitcoin observation used by PRICEB.TC. BTC/USD comes from Coinbase Exchange; other currencies are indicative daily FX conversions. No API key is required.</p><h2>Get an observation</h2><p><code>GET /api/price?currency=USD</code></p><p><a href="/api/price?currency=USD">Open the current BTC/USD JSON response</a></p><h2>Response example</h2><p>This server-rendered example is the observation available when this document was requested. Check its market timestamp before using it.</p><!--API_OBSERVATION--><noscript><p>The JSON endpoint works without JavaScript.</p></noscript><h2>Fields</h2><dl>${fields.map(([name, description]) => `<dt><code>${escapeHtml(name)}</code></dt><dd>${escapeHtml(description)}</dd>`).join("")}</dl><h2>Freshness and errors</h2><p>Responses use Cache-Control: no-store. Each request reads the latest in-memory observation; it does not guarantee a new market trade. There is no single global Bitcoin price.</p><p>HTTP 400: INVALID_CURRENCY. HTTP 503: PRICE_UNAVAILABLE. Errors use code and message fields. Rate limiting can return HTTP 429; back off before retrying. The service currently limits requests to 120 per minute per IP across routes.</p><h2>Related resources</h2><p><a href="/bitcoin-price.md">Bitcoin price as Markdown</a> · <a href="/api/currencies">Supported currencies</a> · <a href="/bitcoin-price-updates">Source, updates and history API</a> · <a href="/studio">Widget Studio</a></p></main><footer class="public-footer"><a href="/about">About</a><a href="/faq">FAQ</a><a href="/">PRICEB.TC</a><a href="mailto:${siteContent.contactEmail}">Contact</a></footer></div>`;
}
...
const pages = [
  ...
  {
    filename: "api/index.html", staticOnly: true,
